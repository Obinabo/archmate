...
include "includes/nav-menu.php";
    ...
    if($_SERVER['REQUEST_METHOD'] == 'POST'){
        ...
        if(isset($_POST['submit']) && $_POST['submit'] == 'delete'){
            $q6 = "DELETE FROM affiliatesales WHERE id = ?";
                $stmt6 = mysqli_prepare($con, $q6);
                mysqli_stmt_bind_param($stmt6, 'i', $id);
                mysqli_stmt_execute($stmt6);
            if(mysqli_stmt_affected_rows($stmt6) == 1){
                $successmsg = '<div class="success"> Sale successfully Deleted, Refreshing...</div>';            
...
<h3> Delete Sale Record</h3><hr>
    ...
    <p>Clicking on DELETE will automatically remove this sale and its earning record from the database...</p>
    <p>Your account balance will not be affected by a DELETE action.</p>
            ...
            <div class="container2">
                <?php 
                    ...
                    if(isset($successmsg)){echo $successmsg; redirect('./sales', 3);}
                echo '<h3 align-center">Post Title: '.$row['title'].' </h3>';
                ?>
